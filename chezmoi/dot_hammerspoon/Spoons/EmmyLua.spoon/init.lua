...
function M:init()
  ...
  for _, spoon in ipairs(hs.spoons.list()) do
    ...
    -- Load custom modules (hsLauncher, hsStringEval)
    local customModules = {
        { path = os.getenv("HOME") .. "/Scripts/Metascripts/hsLauncher/docs.json",   prefix = "launcher." },
        { path = os.getenv("HOME") .. "/Scripts/Metascripts/hsStringEval/docs.json", prefix = "stringeval." },
    }
    for _, module in ipairs(customModules) do
        if hs.fs.attributes(module.path, "modification") then
            M.createWhenChanged(module.path, module.prefix)
        end
    end
  M.writeTimestamps()
